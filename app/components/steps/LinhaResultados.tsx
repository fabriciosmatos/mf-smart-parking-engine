...
interface PropriedadesLinhaResultados {
  unidade: Unit;
  vaga: ParkingSpace;
  atribuicao: Assignment;
}

export const LinhaResultados: React.FC<PropriedadesLinhaResultados> = ({
  unidade,
  vaga,
  atribuicao
}) => {
    ...
    <tr className="hover:bg-slate-50 transition-all group">
      ...
      <td className="px-4 md:px-6 lg:px-10 py-6 text-center">
        <div className="inline-flex flex-col items-center">
          ...
          <div className="flex gap-1.5 mt-2">
            <span
              title={vaga.coverage === 'COVERED' ? 'Vaga Coberta' : 'Vaga Descoberta'}
              className={`px-2 py-0.5 rounded-md text-[8px] font-black cursor-help uppercase shadow-sm ${
                vaga.coverage === 'COVERED' ? 'bg-indigo-100 text-indigo-700' : 'bg-orange-100 text-orange-700'
              }`}
            >
              {vaga.coverage === 'COVERED' ? 'COB' : 'DES'}
            </span>
            <span
              title={vaga.access === 'FREE' ? 'Acesso Livre (sem manobra)' : 'Vaga Presa (necessita manobra)'}
              className={`px-2 py-0.5 rounded-md text-[8px] font-black cursor-help uppercase shadow-sm ${
                vaga.access === 'FREE' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'
              }`}
            >
              {vaga.access === 'FREE' ? 'LIV' : 'PRE'}
            </span>
          </div>
        </div>
      </td>
      <td className="px-4 md:px-6 lg:px-10 py-6">
        <div className="flex flex-wrap gap-2 max-w-md">
          {atribuicao.rulesApplied.map((rule, idx) => {
      ...
      <td className="px-4 md:px-6 lg:px-10 py-6 text-right">
        <div className={`font-mono font-black text-xl italic leading-none ${
          atribuicao.score >= 5000 ? 'text-indigo-600' : 'text-slate-900'
        }`}>
          {atribuicao.score.toFixed(1)}
        </div>
      </td>
    </tr>
  );
